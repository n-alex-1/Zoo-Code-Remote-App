...
/**
 * Session 7: list of all modes (built-in + custom). The active mode is highlighted; tapping a row
 * switches the extension's mode (`POST /api/mode`) with optimistic UI and rollback on error.
 */
...
fun ModeScreen(
	...
	var pendingSnackbar by remember { androidx.compose.runtime.mutableStateOf<String?>(null) }
	...
	Scaffold(
		...
		when {
			...
			state.loadError != null && state.modes.isEmpty() -> Box(modifier = Modifier.fillMaxSize().padding(padding), contentAlignment = Alignment.Center) {
				...
				items(state.modes, key = { it.slug }) { mode ->
					ModeRow(
						mode = mode,
						active = state.pendingSlug == mode.slug || (state.pendingSlug == null && state.currentSlug == mode.slug),
						busy = state.pendingSlug != null && state.pendingSlug != mode.slug,
						onClick = { viewModel.select(mode.slug) },
					)
				}
			}
		}
